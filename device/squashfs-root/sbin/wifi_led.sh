...
led1=/sys/class/leds/blue1_led
pre_state=/tmp/wifi_led_state
mode=$1
brightness=$2
delay_off=$2
delay_on=$3
...
light_on_led()
{
	...
	if [ ${simulate} -eq 1 ];then
		...
		echo ${default_br} > ${led1}/brightness
	fi
}
...
light_off_led()
{
	...
	if [ ${simulate} -eq 1 ];then
		echo "none" > ${led1}/trigger
		echo ${default_off} > ${led1}/brightness
	fi
}
...
blink_led()
{
	...
	if [ ${simulate} -eq 1 ];then
		light=`cat ${led1}/brightness`
		if [ "$light" -eq "0" ]
		then
			light_on_led 
		fi
	
		echo "timer" > ${led1}/trigger
		echo $delay_off > ${led1}/delay_off
		echo $delay_on > ${led1}/delay_on
	fi
}
